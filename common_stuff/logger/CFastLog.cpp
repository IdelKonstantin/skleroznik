...
namespace
{
    ...
    /* Формат метки времени [09-12-2019 00:09:59.598514] */
	...
	strftime(buffer, 32, "%d-%m-%Y %T.", gmtime);
	char microseconds[7];
...
namespace fastlog
{
    ...
    char const * to_string(LogLevel loglevel)
    {
        ...
        {
            case LogLevel::INFO:
                return fastlog::LOG_INFO_TAG.c_str();
            case LogLevel::WARN:
                return fastlog::LOG_WARNING_TAG.c_str();
            case LogLevel::CRIT:
                return fastlog::LOG_ERROR_TAG.c_str();
        }
        return "XXXX";
    }
    ...
    CFastLogLine::CFastLogLine(LogLevel level, char const * file, char const * function, uint32_t line)
	: m_bytes_used(0)
    ...
    void CFastLogLine::stringify(std::ostream & os)
    {
    ...
    void CFastLogLine::stringify(std::ostream & os, char * start, char const * const end)
    {
        if (start == end)
            return;
    ...
    void CFastLogLine::resize_buffer_if_needed(size_t additional_bytes)
    {
    ...
    void CFastLogLine::encode_c_string(char const * arg, size_t length)
    {
        if (length == 0)
            return;
    ...
    class QueueBuffer : public BufferBase
    {
        ...
        void roll_file()
        {
            ...
            {
                //std::cerr << e.what() << '\n';
                std::cout << "Ошибка при открытии файла " << log_file_name << " - "<< e.what() << '\n';
                throw;
            }
            std::cout << log_file_name.c_str() << std::endl;
    ...
    {
        public:
        CFastLogger(NonGuaranteedLogger ngl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb)
            : m_state(State::INIT)
        ...
        CFastLogger(GuaranteedLogger gl, std::string const & log_directory, std::string const & log_file_name, uint32_t log_file_roll_size_mb)
            : m_state(State::INIT)
        ...
        void pop()
        {
            ...
            CFastLogLine logline(LogLevel::INFO, nullptr, nullptr, 0);
    ...
    bool CFastLog::operator==(CFastLogLine & logline)
    {
	    atomic_fastlogger.load(std::memory_order_acquire)->add(std::move(logline));
	    return true;
    }
    ...
    {
	    fastlogger.reset(new CFastLogger(ngl, log_directory, log_file_name, log_file_roll_size_mb));
    ...
    {
	    fastlogger.reset(new CFastLogger(gl, log_directory, log_file_name, log_file_roll_size_mb));
	    atomic_fastlogger.store(fastlogger.get(), std::memory_order_seq_cst);
    }
